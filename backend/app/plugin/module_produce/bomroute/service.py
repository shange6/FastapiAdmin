...
class ProduceBomRouteService:
    ...
    async def upsert_batch_bomroute_service(cls, auth: AuthSchema, data: list) -> dict:
        ...
        async with async_db_session() as session:
            ...
            sql = text(f"SELECT bom_id, `route` FROM produce_bom_route WHERE bom_id IN ({placeholders})")
            ...
            for item in data:
                ...
                if bom_id not in existing:
                    to_insert.append({"bom_id": bom_id, "route": route})
                elif existing[bom_id] != route:
                    to_update.append({"bom_id": bom_id, "route": route})
            if to_insert:
                ins_sql = text("INSERT INTO produce_bom_route (bom_id, `route`) VALUES (:bom_id, :route)")
                await session.execute(ins_sql, to_insert)
            if to_update:
                upd_sql = text("UPDATE produce_bom_route SET `route` = :route WHERE bom_id = :bom_id")
