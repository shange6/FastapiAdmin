...
class WTBOMS():
    ...
    class WTBOM(object):
        ...
        class BaseBOM(object):
            ...
            def check_all(self):
                ...
                self.check_material()
                self.check_remark()
                self.set_project_code(self.project_code)
                self.set_first_code(self.first_code)
                self.convert_x()
                self.convert_y()
            ...
            def set_project_code(self, project_code: str):
                self.bom["project_code"] = project_code

            def set_first_code(self, first_code: str):
                self.bom["first_code"] = first_code
        ...
        class ProcureBOM(BaseBOM):
            ...
            def get_parent_code(self, code: str):
                self.bom["parent_code"] = self.first_code

            def check_all(self):
                super().check_all()
                self.get_parent_code(self.code)
        ...
        class MiddleBOM(BaseBOM):
            ...
            def get_parent_code(self, code: str) -> str:
                # 获取父代号的后半段
        ...
        class BorrowBOM(MiddleBOM):
            ...
            def get_parent_code(self, code: str):
                # 借用件获取父代码
                if code == self.first_code:  # 如果是首行借用，则不管是否标注父代码后半段
                    parent_code = super().get_parent_code(code)
                    if parent_code:     # 如果self.code有符合条件的父代码
                        self.bom["parent_code"] = parent_code
                    else:   # 如果没有父代码, 比如self.code是另一个项目代号, 则父代码为本项目代号
                        self.bom["parent_code"] = self.project_code
                else:   # 如果不是首行借用, 则获取备注的借用方代号后半段，如果没有备注则为空表示上级有借用
                    match = re.search(self.pattern_char, self.remark)
                    parent_sub_code = match.group(0) if match else None
        ...
        class SpecialBOM(MiddleBOM):
            ...
            def get_parent_code(self, code: str):
                # 获取专用件父代码
                parent_sub_code = super().get_parent_code(code)
    ...
    def __init__(self, data: Dxf2List):
        ...
        first_code = self.boms[0]["code"]
        parts = first_code.split(".")
        if len(parts) > 1:
            part0 = parts[0]
            part1 = re.match(r'\d+', parts[1]).group()
            self.first_code = f"{part0}.{part1}"            
        else:
            self.first_code = self.project_code
        self.check_all()
