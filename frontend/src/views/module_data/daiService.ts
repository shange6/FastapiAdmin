...
/**
 * 逻辑 E：获取工单缺失的项目列表 (工时数 - 工单数)
 */
export async function getMissingOrderProjectList(params: any) {
  // 1. 获取所有项目（不使用分页）
  const queryParams = { ...params };
  delete queryParams.page_no;
  delete queryParams.page_size;
  
  const response = await DataProjectAPI.getAllDataProject(queryParams);
  const projects = response.data.data || [];

  if (projects.length === 0) return [];

  // 2. 批量获取每个项目的缺失工单数量差额
  const projectIds = projects.map((item: any) => item.id).filter(Boolean);
  const missingRes = await ProduceBomManhourAPI.summaryBatchMissingOrderCount(projectIds);
  const missingCountMap = missingRes.data?.data || {};

  // 3. 映射到 dai_project 字段
  return projects.map((item: any) => ({
    ...item,
    dai_project: item.id ? (missingCountMap[String(item.id)] || 0) : 0
  }));
}
