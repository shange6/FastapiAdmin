...
const transformRoutes = (routes: RouteVO[], isTopLevel: boolean = true): RouteRecordRaw[] => {
  return routes.map((route) => {
    ...
    if (normalizedRoute.children && normalizedRoute.children.length > 0) {
      ...
      // 叶子路由
      const component = normalizedRoute.component
        ? modules[`../../views/${normalizedRoute.component}.vue`] ||
          modules["../../views/error/404.vue"]
        : modules["../../views/error/404.vue"];

      if (isTopLevel) {
        // 如果顶级路由是叶子节点（即没有父级目录直接显示在顶级），
        // 也需要用 Layout 包装，否则侧边栏会消失
        normalizedRoute.component = Layout;
        ...
        // 顶级节点保留路径，但移除 name 和 meta 以避免与子节点冲突
        delete normalizedRoute.name;
        // 注意：不删除 meta，因为菜单渲染需要顶级 meta 中的 title 和 icon
      } else {
